    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="bg-[var(--primary)] text-white px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 hover:bg-[#5a0bb0] transition"
        >
          <FiPlus /> Create Plan
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map(plan => (
            <div key={plan._id} className={`bg-white rounded-2xl p-6 border-2 transition-all ${plan.isActive ? 'border-gray-100 hover:border-[var(--primary)]' : 'border-gray-200 opacity-60'}`}>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-[var(--primary)] uppercase tracking-wide text-xs mt-0.5">{plan.targetAudience || "STUDENT"}</span>
                </div>
                <div className="flex justify-between text-sm">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[var(--primary)] outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Price (paise, e.g., 9900 = ₹99)</label>
                  <input type="number" required value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[var(--primary)] outline-none" />
                </div>
              </div>
              
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Description</label>
                <input type="text" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[var(--primary)] outline-none" placeholder="Short description for users" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Max Sessions (Leave empty for unlimited)</label>
                  <input type="number" placeholder="Unlimited" value={formData.maxSessions} onChange={e => setFormData({...formData, maxSessions: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[var(--primary)] outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Max Profile Views</label>
                  <input type="number" placeholder="Unlimited" value={formData.maxProfileViews} onChange={e => setFormData({...formData, maxProfileViews: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[var(--primary)] outline-none" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.period} onChange={e => setFormData({...formData, period: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[var(--primary)] outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Target Audience</label>
                  <select value={formData.targetAudience} onChange={e => setFormData({...formData, targetAudience: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[var(--primary)] outline-none">
              ...
              <div className="flex items-center mt-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={formData.isActive} onChange={e => setFormData({...formData, isActive: e.target.checked})} className="w-5 h-5 accent-[var(--primary)]" />
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button type="submit" className="px-4 py-2 bg-[var(--primary)] text-white font-semibold rounded-xl hover:bg-[#5a0bb0] shadow-md transition">Save Plan</button>
              </div>
            </form>
          </div>
        </div>
